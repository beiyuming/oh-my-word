...
5. 汇报证据和限制。
6. 只有用户明确要求时才提交；用户已设定的发布打包规则属于明确要求，见 Git 纪律。
...
- 发布打包例外：每次完成“更新并重新打包安装包”后，必须在验证通过后执行一次 Git 提交并推送到 GitHub。
...
- 发布提交信息应包含版本号，例如 `release: v0.1.1`。推送失败、远端缺失或认证失败时，必须明确报告失败原因和当前提交状态。
- 发布安装包不通过普通 Git 跟踪 `dist/` 二进制产物；每次版本发布在 Git 提交和 `git push` 成功后，还必须创建或更新同版本 GitHub Release，并上传带版本号的安装包附件，例如 `oh-my-word-setup-v0.1.1.exe`。
- 如果 GitHub Release 创建/上传因 `gh` 缺失、未登录、token 缺失、网络或权限失败而无法完成，必须明确报告失败原因、已推送的 commit/tag 状态、安装包本地路径，并给出可手动执行的 `gh release create` 或 `gh release upload` 命令。
- 除非用户明确要求，不要改写历史。
