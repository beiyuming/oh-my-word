...
- 如果继续打包工作，编辑前检查 `build/`、`oh-my-word-py.spec` 和当前未跟踪安装器文件。当前轻量安装器已能生成带版本号的安装包，且 VoxCPM 旧下载/安装 UI 已从安装器移除；真实安装/卸载交互仍需单独验证。用户已要求：每次完成更新安装包打包后，必须只提交本次发布相关变更、推送到 GitHub、创建同版本 tag，并创建带安装包 asset 的 GitHub Release；默认不要提交 `dist/` 产物或无关 dirty 文件。如果 `gh`/token/网络/权限不可用，报告失败原因并给出手动 `gh release create/upload` 命令。
- 如果继续 VoxCPM TTS 工作，当前目标是本机 optional companion process：主程序设置支持 `system_qt` 和 `voxcpm_local`，VoxCPM 通过本地 HTTP endpoint 调用 `tools/voxcpm_service`，主安装器不再提供 VoxCPM 下载/安装 UI。仍需在安装了 runtime 包和模型包的机器上验证真实 VoxCPM 合成播放。
- 如果继续 VoxCPM 应用内管理工作，重点文件是 `app/voxcpm_service.py`、`app/settings_window.py`、`app/controller.py`、`app/models.py`、`app/settings.py`、`build/build_exe.ps1` 和 `oh-my-word-py.spec`。当前策略是设置页四条下载/导入路径 + 启动/停止/检测/打开日志，`voxcpm_auto_start` 仅在使用时启动已安装服务，不自动安装。
- 如果继续 2026-06-18 这轮 VoxCPM2 分包工作，先看 `docs/superpowers/specs/2026-06-18-voxcpm2-modelscope-download-design.md`、`docs/superpowers/plans/2026-06-18-voxcpm2-modelscope-download.md`、`app/voxcpm_runtime.py`、`app/voxcpm_service.py`、`app/controller.py`、`app/settings_window.py` 与 `tests/test_voxcpm_runtime.py` / `tests/test_voxcpm_service_manager.py`。当前代码已支持运行时包和模型包拆分、ModelScope 下载、手动导入模型包、单独下载模型包、可编辑的 ModelScope 路径字段，以及后台线程执行的下载/导入流程；当前新默认 runtime 资产为 `voxcpm2-runtime-win-x64-cu130-r2.zip`，并要求包内自带 portable Python。真实仓库端到端验证仍未完成。
- 2026-06-19 新结论：真实魔搭 `r2` runtime/model 资产已核对并在隔离目录完成代码级端到端验证，说明“需要宿主 Python”不是包内容问题。真正根因是 Windows 上目录激活 `Path.rename()` 命中 `WinError 5`；`app/voxcpm_service.py` 已改成 `rename` 失败时回退 `shutil.move()`，并新增 runtime/model 两条回归测试。若用户继续反馈导入问题，先确认他运行的是包含这次修复的安装包，再让他提供 `storage\app.log` 与对应 `install.log`。
- 2026-06-20 新结论：应用内 ModelScope 下载失败的另一条根因也已定位并修复——`app/voxcpm_service.py` 还在拼旧的 `api/v1/.../repo?...` 地址，而当前 `borealis/oh-my-word-voxcpm2-runtime` 仓库真实可下载地址是 `https://www.modelscope.cn/models/{namespace}/{repo}/resolve/master/{filename}`。同时，runtime/model 的 blocking 下载 helper 过去会用消息文本里的“下载”误判成功；现已改为显式 `bool` 返回，并补上 runtime zip、runtime sha、model zip、model sha 失败路径测试。若用户在 `v0.1.18+` 仍反馈“点下载没反应/提示成功但实际上没装上”，优先收集 `storage\app.log`、安装目录下 `.downloads` 残留，以及设置页当前 namespace/repo/runtime filename 值。
