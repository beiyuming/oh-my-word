...
- 当前版本号来源为 `app/version.py`，当前值随发布递增；当前工作树版本为 `0.1.10`。
- `build/build_installer.ps1` 默认输出带版本号的安装包，例如 `dist/oh-my-word-setup-v0.1.10.exe`。
...
- `后台安装 / 更新` 继续存在，但只是兼容/兜底路径，不再是普通用户首选。
