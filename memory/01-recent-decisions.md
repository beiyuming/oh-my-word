...
- 安装包二进制随同版本 GitHub Release 发布：普通 Git 提交和 `git push` 成功后，还必须创建或更新同版本 Release，并上传带版本号的安装包附件；如果 `gh`/token/网络/权限不可用，必须报告失败原因和手动上传命令。
...
- 当前版本号来源为 `app/version.py`，当前值 `0.1.1`。
- `build/build_installer.ps1` 默认输出带版本号的安装包，例如 `dist/oh-my-word-setup-v0.1.1.exe`。
- 设置窗口必须包含“关于”页，显示当前版本和更新日志。
- 后续每次重新打包发布都应先更新版本号和更新日志，再构建安装包。
- 每次完成更新安装包打包后，还应以包含版本号的提交信息提交本次发布相关变更，并推送到 GitHub。
