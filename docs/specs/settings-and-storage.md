...
在保留旧版 `后台安装 / 更新` 入口的同时，发音分类应新增 `导入 VoxCPM 运行时包` 主入口，用于导入来自 GitHub Release 的预构建 runtime zip。该入口不新增持久化设置字段，但设置页状态区必须展示运行时导入状态和只读元信息，例如 runtime ID、CUDA 标签、最低驱动要求和模型版本。旧版脚本安装路径视为兼容/兜底路径，不再是普通用户的首选入口。
