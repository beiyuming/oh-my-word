...
设置窗口必须按类别组织：学习、显示、发音、快捷键、词库、关于。自动朗读开关、自动朗读延迟、朗读内容、口音、发音引擎、VoxCPM 安装目录、模型目录、镜像开关、使用时自动启动开关、语气提示词、流式预缓冲、安装/启动/停止/检测/打开日志入口属于发音分类。点击发音分类中的 VoxCPM 安装/启动/停止/检测/打开日志入口前，应先读取并应用当前窗口里已编辑但尚未保存的 VoxCPM 路径和镜像设置，避免动作基于过时状态。关于分类必须显示当前应用版本和更新日志；版本来源应来自代码内单一版本源，不应在 UI 中手写另一个版本号。

在保留旧版 `后台安装 / 更新` 入口的同时，发音分类应新增 `下载并导入运行时包` 主入口，用于从 ModelScope 拉取预构建 runtime zip 与 model zip 并顺序导入；同时保留 `导入 VoxCPM 运行时包` 和 `导入模型包` 作为手动下载后的兜底入口。上述入口不新增额外持久化设置字段，但设置页状态区必须展示运行时导入状态和只读元信息，例如 runtime ID、CUDA 标签、最低驱动要求和模型版本。旧版脚本安装路径视为兼容/兜底路径，不再是普通用户的首选入口。
